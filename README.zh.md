...
## 🔬 这个 skill 到底好在哪里（不是又一个书单）

**每本书都按 5 个角度 distill，不是 1 个角度**。这是这个 skill 真正的架构选择 —— 也是它跟"我读了一本书，给你做个 summary"的根本差别。
...
```
   ...
   │          │        │        │          │
   ▼          ▼        ▼        ▼          ▼
 公司战略    创业战术   个人现金流  失败红线   30/60/90 行动
   │          │        │        │          │
   └──────────┴────────┼────────┴──────────┘
                       │
                       ▼
            + AI 时代改写 + Cross-book stacking
```

### 样本：《影响力》(Cialdini #52) 实际输出

> "Founder 的工作不是'讲清楚价值'——是把 6 个心理快捷键按顺序点亮。销售周期长度 = 6 原则部署完整度的倒数。"

| 角度 | 这个 skill 返回什么 |
|---|---|
| 🏢 **公司战略** | Cold email 第一封必带 specific report (互惠激活), CTR ↑ 3x |
| 🚀 **创业战术** | 目标：6 原则完整部署 → 销售周期 90 天 → 30 天 |
| 👤 **个人** | 6 原则也是 BS 探测器 —— 看出别人正在用哪条对你 |
| ⚠️ **红线** | 用过度 = 操纵 = 失信反弹 5 倍 |
| ⏱ **48h 行动** | Audit 你最近 5 封 cold email — 标注每封激活了哪条原则 |
| 🤖 **AI 时代** | LLM 让"个性化互惠"成本降到接近零，但过度个性化会触发 creepy alarm |

**一本书。六个独立透镜。不是哲学——是操作手册。**

### 来自原书的金句（原文 · 非我转述）

以下不是我的解读，是**原作者本人的句子**——直接从书里 pull 出来。这个 skill 是建立在真实阅读之上，不是 Wikipedia 摘要。
...
> — 一个研究影响力 30 年的心理学家，开篇就承认自己曾经是受害者。**这就是为什么要读原书**。

**这些句子，安装 skill 之后立刻进入你的 stack**。不是大意。是原句、原语境、原断言——distilled but not diluted（蒸馏过，但没稀释）。

### 这对你意味着什么

大多数"AI 读书 summary"工具给你的是 **这本书的目录 · 重新措辞一遍**。

Reading Compass 给你的是：
- 每本书 12-15 KB 结构化笔记（4 part · 9 subsection）
- 每个框架的 5+ 角度：**公司 / 创业 / 个人 / 失败 / 行动**
- Cross-book stacking 表面化书与书之间的张力
- AI 时代重写（每个框架都被重新问一次："post-GPT 这条还成立吗？"）
- 具体 30/60/90 天行动清单，不是模糊原则

→ 当你问罗盘一个决策问题，你拿到的不是一本书的扁平答案。  
你拿到的是 **5 本书互相 productively 吵架 + 一个 committed verdict**。
